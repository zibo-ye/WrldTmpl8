...
)
{
	...
	else
	{
		...
		const float3 primaryHitPoint = hit->rayDirection * hit->distance + params->E;
		...
		if (distance(length(R), dist2) < 0.8)
		{
			...
			const float3 directContribution = ToFloatRGB(emitterVoxel) * EmitStrength(emitterVoxel) * NdotL / (length(R) * length(R));
